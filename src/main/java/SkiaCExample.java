import org.bytedeco.skia.global.Skia;
...
public class SkiaCExample {
    ...
    private static class HelloTriangleApplication {
        ...
        private void createSurface() {
            ...
            gr_vk_backendcontext_t vk_backendcontext_t = new gr_vk_backendcontext_t();

            gr_context_t directContext =   gr_context_make_vulkan(vk_backendcontext_t);

            //image info

            gr_vk_imageinfo_t imageinfoT = new gr_vk_imageinfo_t()
                    .fImage(23)
                    .fImageTiling(VK_IMAGE_TILING_OPTIMAL)
                    .fImageLayout(VK_IMAGE_LAYOUT_GENERAL)
                    .fFormat(VkSurfaceFormat2KHR.PNEXT)
                    .fLevelCount(1);

            //backend render taget
            gr_backendrendertarget_t backendrendertarget_t =
                    gr_backendrendertarget_new_vulkan(800, 600, 0,
                            imageinfoT);
            System.out.println(backendrendertarget_t);
            sk_colorspace_t colorspace_t = new sk_colorspace_t();
            sk_surfaceprops_t sk_surfaceprops_t = new sk_surfaceprops_t();
            sk_surface_t sk_surface_te = sk_surface_new_backend_render_target(directContext,backendrendertarget_t,
                    0,8,colorspace_t,sk_surfaceprops_t);
            System.out.println(sk_surface_te);

// do not .close() — Surface manages its lifetime here
          /*  LongBuffer pSurface = sk_surface_te.asByteBuffer().asLongBuffer().asReadOnlyBuffer(); // .longs(VK_NULL_HANDLE);
           ...
           sk_canvas_t canvas = sk_surface_get_canvas(sk_surface_te);
            draw(canvas);*/
        ...
        private void createGraphicsPipeline() {
            ...
            try(MemoryStack stack = stackPush()) {
                ...
                ShaderSPIRVUtils.SPIRV vertShaderSPIRV = ShaderSPIRVUtils.compileShaderFile("shaders/09_shader_base.vert", ShaderSPIRVUtils.ShaderKind.VERTEX_SHADER);
                ShaderSPIRVUtils.SPIRV fragShaderSPIRV = ShaderSPIRVUtils.compileShaderFile("shaders/09_shader_base.frag", ShaderSPIRVUtils.ShaderKind.FRAGMENT_SHADER);
