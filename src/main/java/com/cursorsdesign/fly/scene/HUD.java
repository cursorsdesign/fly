...
public class HUD {
    ...
    public void draw(Canvas canvas, Frame frame, int width, int height) {
        ...
        FontMetrics metrics = Frame.inter13.getMetrics();

        float variantsHeight = frame._variants.length  > 1 ? 25 : 0;
        ...
        Frame.drawStringCentered(canvas, "←→", 15, 10, Frame.inter13, fg);
        int sceneIdx = 1;
        for (String sceneKey: Navigation.scenes.keySet()) {
            if (sceneKey.equals(Navigation.currentScene)) break;
            sceneIdx++;
        }
        Frame.drawStringLeft(canvas, sceneIdx + "/" + Navigation.scenes.size() + " " + Navigation.currentScene, labelBounds, Frame.inter13, fg);
        canvas.translate(0, 25);

        // Variants
        if (frame._variants.length > 1) {
            labelBounds = Rect.makeLTRB(35, 0, 225, 20);
            canvas.drawRRect(buttonBounds, bg);
            Frame.drawStringCentered(canvas, "↑↓", 15, 10, Frame.inter13, fg);
            Frame.drawStringLeft(canvas, (frame._variantIdx + 1) + "/" + frame._variants.length + " " + frame.variantTitle(), labelBounds, Frame.inter13, fg);
            canvas.translate(0, 25);
        }
        ...
        canvas.drawRRect(buttonBounds, bg);
        Frame.drawStringCentered(canvas, "V", 15, 10, Frame.inter13, fg);
        Frame.drawStringLeft(canvas, "VSync: " + (Navigation.vsync ? "ON" : "OFF"), labelBounds, Frame.inter13, fg);
        canvas.translate(0, 25);

        // Stats
        canvas.drawRRect(buttonBounds, bg);
        Frame.drawStringCentered(canvas, "S", 15, 10, Frame.inter13, fg);
        Frame.drawStringLeft(canvas, "Stats: " + (Navigation.stats ? "ON" : "OFF"), labelBounds, Frame.inter13, fg);
        canvas.translate(0, 25);

        // GC
        canvas.drawRRect(buttonBounds, bg);
        Frame.drawStringCentered(canvas, "G", 15, 10, Frame.inter13, fg);
        Frame.drawStringLeft(canvas, "GC objects: " + allocated, labelBounds, Frame.inter13, fg);
        canvas.translate(0, 25);

        // Native calls
        Frame.drawStringLeft(canvas, "Native calls: " + nativeCalls, labelBounds, Frame.inter13, fg);
        canvas.translate(0, 25);
        ...
        Frame.drawStringLeft(canvas, time, Rect.makeLTRB(times.length + 5, 0, 225, 20), Frame.inter13, fg);
        String fps = String.format("%.0f fps", 1000.0 / Arrays.stream(times).takeWhile(t->t>0).average().getAsDouble());
        Frame.drawStringLeft(canvas, fps, Rect.makeLTRB(times.length + 5, 25, 225, 40), Frame.inter13, fg);
        canvas.translate(0, 25);
